...
def fit_tracks(track_candidates_file, tracks_file, z_positions, fit_duts=None, ignore_duts=None, include_duts=[-5, -4, -3, -2, -1, 1, 2, 3, 4, 5], track_quality=1, max_tracks=None, output_pdf=None, use_correlated=False, method = "Interpolation", pixel_size = [], geometryFile = 'data/Geometry.h5'):
                ...
                for fit_dut in fit_duts:  # Loop over the DUTs where tracks shall be fitted for
                    ...
                    for dut_index in range(0, n_duts):  # Fill index loop of new array
                        if method == "Interpolation" and (1 << dut_index) & dut_selection == (1 << dut_index):  # True if DUT is used in fit
                            xr = good_track_candidates['column_dut_%s' % dut_index]*rotations[dut_index,0,0] + good_track_candidates['row_dut_%s' % dut_index]*rotations[dut_index,0,1] + translations[dut_index,0]
                            yr = good_track_candidates['row_dut_%s' % dut_index]*rotations[dut_index,1,1] + good_track_candidates['column_dut_%s' % dut_index]*rotations[dut_index,1,0] + translations[dut_index,1]
                            ...
                            if (1 << dut_index) & dut_selection == (1 << dut_index):#TOCHECK! Not used = masked, OK, but also DUT must be masked...
                                #xyz = np.column_stack(np.ma.array((good_track_candidates['column_dut_%s' % dut_index], good_track_candidates['row_dut_%s' % dut_index], np.repeat(z_positions[dut_index], n_tracks))))
                                xr = good_track_candidates['column_dut_%s' % dut_index]*rotations[dut_index,0,0] + good_track_candidates['row_dut_%s' % dut_index]*rotations[dut_index,0,1] + translations[dut_index,0]
                                yr = good_track_candidates['row_dut_%s' % dut_index]*rotations[dut_index,1,1] + good_track_candidates['column_dut_%s' % dut_index]*rotations[dut_index,1,0] + translations[dut_index,1]
                    ...
                    if method == "Interpolation":
                        ...
                        tracks_array_k = create_results_array_kalman(good_track_candidates, track_estimates, chi2s, n_duts)
