    ...
    with tb.open_file(input_tracklets_file, mode='r') as in_file_h5:
        ...
        with tb.open_file(output_track_candidates_file, mode='w') as out_file_h5:
            ...
            for tracklets_data_chunk, index in analysis_utils.data_aligned_at_events(tracklets_node, chunk_size=chunk_size):
                ...
                combined = np.column_stack((tracklets_data_chunk['event_number'], tr_x, tr_xerr, tr_y, tr_yerr, tr_z, tr_zerr, tr_charge, tracklets_data_chunk['track_quality'], tracklets_data_chunk['n_tracks']))
...
def fit_tracks(input_track_candidates_file, input_alignment_file, output_tracks_file, fit_duts=None, selection_hit_duts=None, selection_fit_duts=None, exclude_dut_hit=True, selection_track_quality=1, max_tracks=None, force_prealignment=False, use_correlated=False, min_track_distance=False, chunk_size=1000000):
    ...
    def store_track_data(fit_dut, min_track_distance, track_hits=None):  # Set the offset to the track intersection with the tilted plane and store the data
        ...
        plot_utils.plot_track_chi2(chi2s=chi2s, fit_dut=fit_dut, output_pdf=output_pdf, track_hits=track_hits)
            ...
            with tb.open_file(output_tracks_file, mode='w') as out_file_h5:  # Append mode to be able to append to existing tables; file is created here since old file is deleted
                ...
                for fit_dut_index, actual_fit_dut in enumerate(fit_duts):  # Loop over the DUTs where tracks shall be fitted for
                    ...
                    for track_candidates_chunk, index_candidates in analysis_utils.data_aligned_at_events(in_file_h5.root.TrackCandidates, chunk_size=chunk_size):
                        ...
                        track_hits_err = np.full((n_tracks, n_fit_duts, 3), np.nan)
                        dut_hits = np.full((n_tracks, 3), np.nan)
                        dut_hits_err = np.full((n_tracks, 3), np.nan)
                        ...
                        for dut_index in range(0, n_duts):  # Fill index loop of new array
                            if ((1 << dut_index) & dut_fit_selection) == (1 << dut_index):  # True if DUT is used in fit
                                ...
                                xyz_err = np.column_stack((good_track_candidates['xerr_dut_%s' % dut_index], good_track_candidates['yerr_dut_%s' % dut_index], good_track_candidates['z_dut_%s' % dut_index]))#TODO: error on z?
                                track_hits[:, index, :] = xyz
                                track_hits_err[:, index, :] = xyz_err
                                index += 1
                            if dut_index == fit_dut_index:
                                xyz = np.column_stack((good_track_candidates['x_dut_%s' % dut_index], good_track_candidates['y_dut_%s' % dut_index], good_track_candidates['z_dut_%s' % dut_index]))
                                xyz_err = np.column_stack((good_track_candidates['xerr_dut_%s' % dut_index], good_track_candidates['yerr_dut_%s' % dut_index], good_track_candidates['z_dut_%s' % dut_index]))#TODO: error on z?
                                dut_hits[:,:] = xyz
                                dut_hits_err[:,:] = xyz_err

                        # Split data and fit on all available cores
                        n_slices = cpu_count()
                        slices = np.array_split(track_hits, n_slices)
                        slices_err = np.array_split(track_hits_err, n_slices)
                        kwargs = []
#                        for cpu in range(n_slices):
#                            kwargs[cpu] = []
                        for i, _ in enumerate(slices):
                            kwargs.append({'track_hits': slices[i], 'track_hits_err' : slices_err[i]})
                        results = pool.map(_fit_tracks_loop, kwargs)
                        del track_hits
                        ...
                        if not same_tracks_for_all_duts:  # Check if all DUTs were fitted at once
                            store_track_data(actual_fit_dut, min_track_distance, track_hits=dut_hits)
                        else:
                            for dut_index in fit_duts:
                                store_track_data(dut_index, min_track_distance, track_hits=dut_hits)
                        del dut_hits
...
def _fit_tracks_loop(track_hits_all):
    ''' Do 3d line fit and calculate chi2 for each fit. '''
    track_hits = track_hits_all['track_hits']
    track_hits_err = track_hits_all['track_hits_err']
    def line_fit_3d(hits, hits_err=None):
        ...
        if hits_err is not None:
            weight = np.zeros(shape=hits_err.shape)
            weight[:,0] = np.asarray([1/hits_err[i,0] if np.isfinite(hits_err[i,0]) else 0.000001 for i in range(hits_err.shape[0])])
            weight[:,1] = np.asarray([1/hits_err[j,1] if np.isfinite(hits_err[j,1]) else 0.000001 for j in range(hits_err.shape[0])])
            weight[:,2] = np.asarray([0 for k in range(hits_err.shape[0])])
            pars_x, cov_x = np.polyfit((hits-datamean).T[2][:], (hits-datamean).T[0][:], 1, w=weight[:,0], full=False, cov=True)#weights as inverse of the error
            pars_y, cov_y = np.polyfit((hits-datamean).T[2][:], (hits-datamean).T[1][:], 1, w=weight[:,1], full=False, cov=True)#note: they are squared in the algorithm
            pars_x_err = cov_x.dot(pars_x)
            pars_y_err = cov_y.dot(pars_y)
        
        slope_err = np.zeros(shape=slope.shape)
        offset_err = np.zeros(shape=offset.shape)
        if hits_err is not None:
            slope[0] = pars_x[0]
            slope[1] = pars_y[0]
            offset[0] = pars_x[1] + datamean[0]
            offset[1] = pars_y[1] + datamean[1]
            slope_err[0] = pars_x_err[0]
            slope_err[1] = pars_y_err[0]
            offset_err[0] = pars_x_err[1]
            offset_err[1] = pars_y_err[1]
        ...
        if hits_err is None:
            ...
            intersections = offset + slope * (hits.T[2][:, np.newaxis] - offset[2])  # Fitted line and DUT plane intersections (here: points)
        if hits_err is None:
            chi2 = np.sum(np.square(hits - intersections), dtype=np.uint32)  # Chi2 of the fit in um
            return datamean, slope / slope_mag, chi2
        else:
            chi2 = np.sum(np.dot(np.square(hits - intersections), np.square(weight.T)))  # Chi2 of the fit in um
    ...
    for index, actual_hits in enumerate(track_hits):  # Loop over selected track candidate hits and fit
        try:
            offset[index], slope[index], chi2[index] = line_fit_3d(actual_hits)#, track_hits_err[index]) #uncomment for using errors in track fit
