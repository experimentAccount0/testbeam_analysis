...
class TestAnalysis(unittest.TestCase):
    ...
    def test_fix_event_alignment(self):  # check with difficult data
        with tb.open_file('./test_alignment_fixing/Tracklets_cut.h5', mode="r+") as in_file_h5:
                particles = in_file_h5.root.Tracklets_cut[:]
                ...
                for index, table_column in enumerate(in_file_h5.root.Tracklets_cut.dtype.names):
                    if 'column_dut' in table_column and 'dut_0' not in table_column:
                        column = np.ascontiguousarray(particles[table_column])
                        row = np.ascontiguousarray(particles['row_dut_' + table_column[-1]])
                        corr = np.ascontiguousarray(np.ones(shape=event_numbers.shape, dtype=np.uint8))  # array to signal correlation to be able to omit not correlated events in the analysis
        ...
        corr, n_fixes = analysis_utils.fix_event_alignment(event_numbers, ref_column, column, ref_row, row, error=1., n_bad_events=3, n_good_events=3, correlation_search_range=100, good_events_search_range=100)
        ...
        self.assertTrue(np.all(corr[0:] == 1))

        # The data is correlated here
        self.assertTrue(np.allclose(ref_column[0:15], column[0:15], atol=2.))
        self.assertTrue(np.allclose(ref_column[17:22], column[17:22], atol=2.))
        self.assertTrue(np.allclose(ref_column[24], column[24], atol=2.))
        self.assertTrue(np.allclose(ref_column[26:48], column[26:48], atol=2.))
