 ...
 * _dx*_dy*_dFrame*number of hits
 *   _dx,_dy are the x,y-step sizes in pixel to search around a hit for additional hits
 *   _dFrame is the Frame window hits are clustered together
 ...
 * - start at one hit position and search around it with a distance of _dx,_dy (8 directions: up, up right, right ...) and _dFrame
 ...
 * - only increase the search distance in a certain direction (until _dx, _dy, _dFrame) if no hit was found in this direction already
...
public:
	...
	void setFrameclusterDistance(const unsigned int& pdFrame);			//sets the Frame depth between two hits that they belong to one cluster
...
private:
	...
	short int* _hitMap;       											//2d hit histogram for each relative Frame (in total 3d, linearly sorted via col, row, rel. Frame)
	...
	unsigned int _maxHitCharge;											//the Charge value a hit is considered to a hit (usually 13)
	...
	unsigned short _actualClusterSeed_relframe;							//temporary value holding the relative Frame number of the seed pixel of the actual cluster
	...
	bool _abortCluster;													//set to true if one cluster Charge hit exeeds _maxClusterHitCharge, cluster is not added to the result array
};
