...
cdef extern from "AnalysisFunctions.h":
    ...
    void fixEventAlignment(const int64_t*& rEventArray, const double*& rRefCol, double*& rCol, const double*& rRefRow, double*& rRow, uint8_t*& rCorrelated, const unsigned int& nHits, const double& rError, const unsigned int& nBadEvents, const unsigned int& nGoodEvents, const unsigned int& goodEventsSearchRange) except +
...
def fix_event_alignment(cnp.ndarray[cnp.int64_t, ndim=1] event_array, cnp.ndarray[cnp.float_t, ndim=1] ref_column, cnp.ndarray[cnp.float_t, ndim=1] column, cnp.ndarray[cnp.float_t, ndim=1] ref_row, cnp.ndarray[cnp.float_t, ndim=1] row, cnp.ndarray[cnp.uint8_t, ndim=1] correlated, const double& error, const unsigned int& n_bad_events, const unsigned int& n_good_events, const unsigned int& good_events_search_range):
    fixEventAlignment(<const int64_t*&> event_array.data, <const double*&> ref_column.data, <double*&> column.data, <const double*&> ref_row.data, <double*&> row.data, <uint8_t*&> correlated.data, <const unsigned int&> event_array.shape[0], <const double&> error, <const unsigned int&> n_bad_events, <const unsigned int&> n_good_events, <const unsigned int&> good_events_search_range)